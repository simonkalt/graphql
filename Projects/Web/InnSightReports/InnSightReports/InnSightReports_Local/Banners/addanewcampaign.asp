<% 	
	...
	If strTask="Edit" Then
		...
		If Application("BanManProMultiSite")=True Then
			set rsTemp=connBanManPro.Execute("Select UserID From Advertisers Where AdvertiserID=" & strAdvertiserID)
			If Not rsTemp.EOF Then
				lngUserID=rsTemp("UserID")
			End If
			Set rsTemp=Nothing
		End If
		...
		strSQLB=strSQLB & " From Banners WHERE (((Banners.AdvertiserID)=" & Clng(strAdvertiserID) & ") AND ( Banners.UserID=" & CLng(Session("BanManProSiteID")) & " Or Banners.UserID=0) AND Banners.AdTextLink<>1) ORDER BY Banners.AdWidth,Banners.AdHeight DESC"

		Set rsBanner=connBanManPro.Execute(strSQLB)
		strSQLB="Select * From CampaignBanners WHERE CampaignBanners.CampaignID=" & Clng(strCampaignID)
...
'End Default campaign ******************************************************************** %>
    ...
    <tr>
      ...
      <td width="366"><font face="Arial" size="2"><select name="CampaignDistribution" size="1">
          <option <% If strTask="Edit" Then %> <% If rsc("CampaignDistribution")="Even" Then%>selected<%End If %><% End If%> value="Even">Evenly
          Distributed Over Flight Dates</option>
          <option <% If strTask="Edit" Then %> <% If rsc("CampaignDistribution")="Weighted" OR rsc("CampaignDistribution")="Normal" Then%>selected<%End If %><%Else%>selected<% End If%> value="Weighted">Weighted
          In Each Zone</option>
    ...
    <tr>
      <td width="134" align="right"><img src="images/required2.gif" WIDTH="14" HEIGHT="12"><font face="Arial" size="2"><strong>Type:</strong></font></td>
      <td width="366"><font face="Arial" size="2"><select name="CampaignType" size="1">
        <option <% If strTask="Edit" Then %> <% If rsc("CampaignType")="CPM" Then%>selected<%End If %><% End If%> value="CPM">CPM
        (Expires by Date or Impressions)</option>
        <option <% If strTask="Edit" Then %> <% If rsc("CampaignType")="Per Click" Then%>selected<%End If %><% End If%> value="Per Click">Per
        Click (Expires based on Date or Clicks)</option>
        <option <% If strTask="Edit" Then %> <% If rsc("CampaignType")="Flat Rate" Then%>selected<%End If %><% End If%> value="Flat Rate">Flat
        Rate (Expires only based on dates)</option>
      </select></font></td>
    </tr>
    <tr>
      <td width="134" align="right"><img src="images/required2.gif" WIDTH="14" HEIGHT="12"><font face="Arial" size="2"><strong>Quantity:</strong></font></td>
      <td width="366"><input type="text" name="CampaignQuantitySold" size="10" <% If strTask="Edit" Then %>value="<%=rsc("CampaignQuantitySold")%>" <%End If%>>
        <font face="Arial" size="1">(Enter actual # of impressions or
        clicks)&nbsp;</font></td>
    </tr>
    <tr>
